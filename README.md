...
> The actual project lives in [`kg-migration-engine/`](kg-migration-engine/) — this
> top-level README exists so it renders on the repo's front page. Full detail, same
> content: [`kg-migration-engine/README.md`](kg-migration-engine/README.md).

## ⚠️ Portfolio Notice

This repository is a **sanitized version** of a professional project. It is shared
to demonstrate coding style, architecture decisions, and technical skills.

The following have been intentionally masked:
- Business logic and domain-specific rules
- Proprietary data models and field names
- Internal API endpoints and credentials
- Company-specific configurations and real project data (sample data ships in
  `kg-migration-engine/data/` in place of the original)

All `[MASKED]` tags in the code indicate where original logic has been replaced
with neutral placeholders. The overall structure, patterns, and implementation
approach are authentic.
...
Full diagrams + request-flow sequence:
[`kg-migration-engine/docs/TECHNICAL_REFERENCE.md`](kg-migration-engine/docs/TECHNICAL_REFERENCE.md#2-architecture).
...
[`kg-migration-engine/src/kgme/api/schemas.py`](kg-migration-engine/src/kgme/api/schemas.py).

## Configuration

Set in `kg-migration-engine/.env` (copy from `.env.example`):
...
├── README.md               this file
└── kg-migration-engine/
    ├── CLAUDE.md .mcp.json  Claude Code control
...
| Doc | For |
|---|---|
| [`kg-migration-engine/docs/IMPLEMENTATION_PLAN.md`](kg-migration-engine/docs/IMPLEMENTATION_PLAN.md) | Full build spec, phase by phase |
| [`kg-migration-engine/docs/TECHNICAL_REFERENCE.md`](kg-migration-engine/docs/TECHNICAL_REFERENCE.md) | Architecture, diagrams, every symbol |
| [`kg-migration-engine/docs/HANDOFF_DETAIL.md`](kg-migration-engine/docs/HANDOFF_DETAIL.md) | Domain background <!-- [MASKED] Internal business context removed --> |
| [`kg-migration-engine/CONTRIBUTING.md`](kg-migration-engine/CONTRIBUTING.md) | Git workflow, zero-experience-required |

## Contributing

See [`kg-migration-engine/CONTRIBUTING.md`](kg-migration-engine/CONTRIBUTING.md) —
written for zero prior GitHub experience. Follow
[`docs/IMPLEMENTATION_PLAN.md`](kg-migration-engine/docs/IMPLEMENTATION_PLAN.md) phase
by phase; don't start a phase before the previous one's Definition of Done is met.
